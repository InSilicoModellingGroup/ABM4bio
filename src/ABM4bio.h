...
void reinit_obstacles(
    ...
    const std::map<int, std::string>& cells,
    const int time,
    const CellMatrixInteraction& cell_matrix_interaction)
{
  /*
  ...
  const bool mechanics_enabled =
    cell_matrix_interaction.AnyCellMatrixMechanicsEnabled(cells);
    ...
    const double contractility_kce_midpoint)
{
   ...
   * Assumption: Higher k_ce gives higher force.
   */
...
int simulate(const std::string& fname, const int seed)
{
  ...
  CellMatrixInteraction cell_matrix_interaction(&params);
  ...
  for (int time=1; time<=n_time; time++)
    { 
      ...
      cell_matrix_interaction.ExportCellPositions(sim, cells, time);
